    ...
    for subdir1 in os.listdir(data_path):
        ...
        for subdir2 in os.listdir(input_dir1):
            ...
            for subdir3 in os.listdir(input_dir2):
                ...
                for img_name in os.listdir(input_dir3):
                    if img_name.startswith(".") or not img_name.endswith(".jpg"): continue
                    
                    # Make output directory
                    out_dir = os.path.join(out, subdir1, subdir2, subdir3)
                    os.makedirs(out_dir, exist_ok=True)
                    # Process image
                    ## Copy output
                    ## Process segmentation and save
                    instance_ids = load_chair_attribute(os.path.join(input_dir3, img_name.replace(".jpg", "_atr.txt")))
                    if len(instance_ids) > -1:
                        shutil.copy(os.path.join(input_dir3, img_name), os.path.join(out_dir, img_name))
                        converted_seg = convert_segmentation(os.path.join(input_dir3, img_name.replace(".jpg", "_seg.png")), instance_ids)
