    ...
    for subdir1 in os.listdir(data_path):
        ...
        for subdir2 in os.listdir(input_dir1):
            ...
            for subdir3 in os.listdir(input_dir2):
                ...
                if not os.path.isdir(input_dir3): continue
                for subdir4 in os.listdir(input_dir3):
                    input_dir4 = os.path.join(input_dir3, subdir4)
                    if not os.path.isdir(input_dir4): continue
                    for img_name in os.listdir(input_dir4):
                        if img_name.startswith(".") or not img_name.endswith(".jpg"): continue
                        
                        # Make output directory
                        out_dir = os.path.join(out, subdir1, subdir2, subdir3, subdir4)
                        os.makedirs(out_dir, exist_ok=True)
                        # Process image
                        ## Copy output
                        ## Process segmentation and save
                        instance_ids = load_chair_attribute(os.path.join(input_dir4, img_name.replace(".jpg", "_atr.txt")))
                        if len(instance_ids) > 0:
                            shutil.copy(os.path.join(input_dir4, img_name), os.path.join(out_dir, img_name))
                            converted_seg = convert_segmentation(os.path.join(input_dir4, img_name.replace(".jpg", "_seg.png")), instance_ids)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ADE20K Chair Category Conversion')
    parser.add_argument('--data_path', type=str, help='path ADE20K/images/training or ADE20K/images/validation')
    parser.add_argument('--out', type=str, help='path ADE20K/images/training or ADE20K/images/validation')
