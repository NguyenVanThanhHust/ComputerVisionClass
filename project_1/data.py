...
class ADE20KLoader(data.Dataset):
    ...
    def transform(self, img, lbl):
        temp_img = cv2.resize(img, dsize=(self.img_size[0], self.img_size[1]))  # uint8 with RGB mode
        img = temp_img
        img = img[:, :, ::-1]  # RGB -> BGR
        ...
        temp_img = cv2.resize(lbl, dsize=(self.img_size[0], self.img_size[1]), interpolation=cv2.INTER_NEAREST)  # uint8 with RGB mode
#        lbl = m.imresize(lbl, (self.img_size[0], self.img_size[1]), "nearest", mode="F")
#        lbl = lbl.astype(int)
        lbl = temp_img.astype(int)
...
if __name__ == "__main__":
    local_path = "../../data/ADE20K_2016_07_26/"
